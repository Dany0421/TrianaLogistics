...
function openPrintWindow(entries, { dateFrom, dateTo }) {
  ...
  const rows = entries.map((log, i) => {
    ...
    return '<tr style="background:' + rowBg + '">'
      ...
      + '<td style="padding:7px 10px"><span style="display:inline-block;padding:2px 7px;border-radius:4px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;background:' + bc.bg + ';color:' + bc.color + '">' + esc(bc.label) + '</span></td>'
