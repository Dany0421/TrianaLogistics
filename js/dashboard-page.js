...
function _renderStatusChart() {
  const allStatuses = ['Active','Waiting for suppliers','Waiting for internal info','Partial responses','Ready for Excel','Pending margin','Awaiting ETA','Awaiting approval','Closed','Cancelled'];
  const allColors   = ['#2563eb','#f59e0b','#22d3ee','#10b981','#7c3aed','#f97316','#38bdf8','#a78bfa','#64748b','#f87171'];
...
async function openCreateModal(id = null) {
  ...
  showModal(`
    ...
    <div class="form-grid-2">
      <div><label>Deadline</label><input type="date" id="f_deadline" value="${esc(p?.deadline||'')}">
        ${p?.created_at ? `<div style="font-size:11px;color:var(--muted);margin-top:4px">Criado a ${new Date(p.created_at).toLocaleDateString('pt-PT',{day:'2-digit',month:'short',year:'numeric'})}</div>` : ''}
      </div>
...
const STANDARD_STATUSES = ['Active','Waiting for suppliers','Waiting for internal info','Partial responses','Ready for Excel','Pending margin','Awaiting ETA','Awaiting approval','Closed','Cancelled'];
function statusBadgeClass(s) {
  const map = {
    'Active':'badge-active','Waiting for suppliers':'badge-waiting','Waiting for internal info':'badge-waiting',
    'Partial responses':'badge-partial','Ready for Excel':'badge-ready','Pending margin':'badge-pending-margin','Awaiting ETA':'badge-awaiting-eta','Awaiting approval':'badge-awaiting-approval','Closed':'badge-closed','Cancelled':'badge-cancelled'
  };
